...
function TransferApproverListPopup() {
  ...
  const doSendData = (userSeq, deptName, userName) => {
    ...
    if ('setUserName' in opener) opener.setUserName(userName);
    if ('setUserSeq' in opener) opener.setUserSeq(userSeq);
    window.close();
  };
  ...
  const init = async () => {
    ...
    const transferUserList = await apiCall(transferUserApi.getTransferUserList, {
      ...parameters,
  ...
  return (
    <>
      <GridItem spacing={2} container divideColumn={5}>
        <MainCard
          title="부서 목록"
          colSpan={1}
          sx={{
...
TransferApproverListPopup.getLayout = function getLayout(page) {
  return (
    <Layout variant="popup" title="사용자 선택">
      {page}
    </Layout>
  );
};
