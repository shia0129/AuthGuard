...
import useRealTimeSslswgUsage from '@modules/hooks/useRealTimeSslswgUsage';
import SslswgStatusChart from '@components/hss/common/dashboardManage/sslswgStatusChart';
import SslswgDenyTopTable from '@components/hss/common/dashboardManage/sslswgDenyTopTable';

function SslswgDashboardPopup({ fullscreenFlag, renderFlag, setRenderFlag }) {
  const intl = useIntl();
  ...
  const { minuteUsage, hourlyUsage, dailyUsage } = useRealTimeSslswgUsage();
        ...
        <GridItem item>
          <Box sx={{ display: 'flex', alignItems: 'center', ml: 5 }}>
            <Typography variant="title2" color="white">
              {/* SSL SWG 모니터링 */}
              {intl.formatMessage({ id: 'monitoring.monitoring-sslswg-title' })}
            </Typography>
        ...
        <GridItem item direction="row" spacing={1}>
          <GridItem item xs={8}>
            <Typography variant="h4" pl={1}>
              {/* 연결량 현황 */}
              {intl.formatMessage({ id: 'monitoring.monitoring-sslswg-connection-status' })}
            </Typography>
            <MainCard
              sx={{
                  ...
                  <Stack direction="row" alignItems="center" justifyContent="space-between">
                    ...
                    <Stack direction="row" alignItems="center">
                      <Typography variant="h6" sx={{ color: '#fff' }}>
                        {/* 1분 연결량 */}
                        {intl.formatMessage({ id: 'monitoring.monitoring-sslswg-connection-1min' })}
                      </Typography>
                    </Stack>
                  </Stack>
                  <Stack
                  ...
                  <Stack direction="row" alignItems="center" justifyContent="space-between">
                    ...
                    <Stack direction="row" alignItems="center">
                      <Typography variant="h6" sx={{ color: '#fff' }}>
                        {/* 1시간 연결량 */}
                        {intl.formatMessage({
                          id: 'monitoring.monitoring-sslswg-connection-1hour',
                        })}
                      </Typography>
                    </Stack>
                  </Stack>
                ...
                <GridItem item xs={6} m={1}>
                  <Stack direction="row" alignItems="center" justifyContent="space-between">
                    ...
                    <Stack direction="row" alignItems="center">
                      <Typography variant="h6" sx={{ color: '#fff' }}>
                        {/* 1일 연결량 */}
                        {intl.formatMessage({ id: 'monitoring.monitoring-sslswg-connection-1day' })}
                      </Typography>
                    </Stack>
                  </Stack>
                  <Stack
          ...
          <GridItem item xs={4}>
            <Typography variant="h4" pl={1}>
              {/* 서비스 상태 */}
              {intl.formatMessage({ id: 'monitoring.monitoring-sslswg-service-status' })}
            </Typography>
            <MainCard
              sx={{
                background: 'rgba(0,0,0,0.3)',
            ...
            >
              <Stack spacing={1} alignItems="center">
                ...
                >
                  {intl.formatMessage({ id: 'monitoring.monitoring-sslswg-status-label' })}{' '}
                  {intl.formatMessage({
                    id: daemonOn
                      ? 'monitoring.monitoring-sslswg-status-running'
                      : 'monitoring.monitoring-sslswg-status-stopped',
                  })}
                </Typography>
              </Stack>
            </MainCard>
              ...
              <GridItem container direction="column" spacing={1}>
                <GridItem item direction="row" justifyContent="space-between" alignItems="baseline">
                  <Typography variant="title1">
                    {/* 허용/차단 현황 */}
                    {intl.formatMessage({ id: 'monitoring.monitoring-sslswg-allow-block-summary' })}
                  </Typography>
                </GridItem>
                <GridItem item>
                  <GridItem item sx={{ height: 350 }}>
                    <SslswgStatusChart isRefreshing={isRefreshing} />
                  </GridItem>
                </GridItem>
              </GridItem>
            </MainCard>
            ...
            >
              <GridItem container direction="column" spacing={1}>
                <GridItem item direction="row" justifyContent="space-between" alignItems="baseline">
                  <Typography variant="title1">
                    {/* 차단 상세 TOP5 */}
                    {intl.formatMessage({ id: 'monitoring.monitoring-sslswg-top-block-detail' })}
                    <Typography component="span" sx={{ fontSize: '13px', color: 'gray', ml: 1 }}>
                      ({intl.formatMessage({ id: 'monitoring.monitoring-sslswg-count-based' })})
                    </Typography>
                  </Typography>
                </GridItem>
                <GridItem item>
                  <GridItem item sx={{ height: 180 }}>
                    <SslswgDenyTopTable isRefreshing={isRefreshing} />
                  </GridItem>
                </GridItem>
              </GridItem>
            </MainCard>
...
SslswgDashboardPopup.getLayout = function getLayout(page) {
  return (
    <Layout variant="monPopup" title="Sslswg Dashboard">
      {page}
    </Layout>
  );
};

export default SslswgDashboardPopup;
