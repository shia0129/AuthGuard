 ...
 *
 */
const ScheduleLogTab = ({ connectionSeq, connectionInfo, handleChangeConnection }) => {
  ...
  const getLogList = async (connectionSeq) => {
    ...
    const result = await apiCall(dbsyncApi.getScheduleLog, {
      ...parameters,
      connectionSeq: connectionSeq,
  ...
  const init = async () => {
    // 테이블, 컬럼정보
    const gridInfo = await HsLib.getGridInfo('DbsyncSchedule', dbsyncApi, openModal);
    if (gridInfo) {
