...
import siteGroupStatusApi from '@api/hss/sslswg/policy/policyDetailManage/siteGroupStatusApi';
import { unstable_batchedUpdates } from 'react-dom';
import Loader from '@components/mantis/Loader';
import { CenteredSpinner } from '@components/modules/common/Spinner';

function SiteGroupUpdateStatusModal({ open, close, modalParams, getStatusList }) {
  // api 호출 함수, openModal 함수
  const [apiCall, openModal] = useApi();
  // 선택한 정책 목록 상태값
  const [selectedSiteIdList, setSelectedSiteIdList] = useState([]);
  // 현재페이지 상태값
  const size = 10;
  ...
  const updateSite = useCallback(async () => {
    ...
    const params = {
      id: id,
      siteIdList: selectedSiteIdList,
    };
    setIsSaving(true);
    const result = await apiCall(siteGroupStatusApi.updateMappingListWithSiteGroupId, params);
    setIsSaving(false);
    if (result) {
      openModal({
  ...
  const handleDelete = (site) => {
    setSelectedSiteIdList(
      selectedSiteIdList.includes(site.id)
        ? selectedSiteIdList.filter((data) => data !== site.id)
        : [...selectedSiteIdList, site.id],
    );
  };
  ...
  const getSiteList = useCallback(
    async ({ silent = true } = {}) => {
      ...
      try {
        ...
        const result = await apiCall(siteGroupStatusApi.getMappingListAllWithSiteGroupId, params);
        ...
        if (result) {
          ...
          unstable_batchedUpdates(() => {
            ...
            if (selectedSiteIdList.length <= 0) {
              const initiallySelectedIds = data
                .filter((site) => site.inUsed === 1)
                .map((site) => site.id);
              setSelectedSiteIdList(initiallySelectedIds);
            }
          });
        }
      } finally {
                    ...
                    options={[
                      {
                        ...
                        color: 'secondary',
                        variant: 'outlined',
                        callBack: () => setSelectedSiteIdList([]),
                      },
                      {
                        label: '전체 선택',
                        color: 'secondary',
                        variant: 'contained',
                        callBack: () => {
                          setSelectedSiteIdList(siteList.map((site) => site.id));
                        },
                      },
                    ]}
                  />
                            ...
                            <ListItemAvatar sx={{ minWidth: '40px' }}>
                              <Avatar
                                sx={{
                                  ...
                                  '& .anticon-site > svg ': { width: '0.8em !important' },
                                }}
                              >
                                <UserOutlined />
                              </Avatar>
              ...
              <GridItem item colSpan={2} sx={{ height: '430px', overflow: 'auto' }}>
                <GridItem direction="row" alignItems="center" sx={{ ml: 1 }}>
                  <GridItem direction="row" sx={{ mb: 1 }}>
                    <Typography variant="subtitle1">사이트 정책 선택&nbsp;&nbsp;&nbsp;</Typography>
                    <Typography color="primary">{selectedSiteIdList.length}</Typography>
                  </GridItem>
                  <GridItem direction="row">
                    ...
                    >
                      {selectedSiteIdList.map((selectedSite) => {
...
SiteGroupUpdateStatusModal.getLayout = function getLayout(page) {
  return <Layout>{page}</Layout>;
};

export default SiteGroupUpdateStatusModal;
