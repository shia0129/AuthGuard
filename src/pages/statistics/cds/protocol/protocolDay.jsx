...
function ProtocolDay() {
  ...
  const [protocolDayList, setProtocolDayList] = useState([
    {
      systemSeq: '시스템',
      statisticalItems: '-',
      date1: '-',
  ...
  const init = async () => {
    const gridInfo = await HsLib.getGridInfo('ProtocolDayList', statisticsApi);
  ...
  const getProtocolDayList = (parameters) => {
    // 일별 목록 조회 api 추가.
  };
    ...
    <GridItem spacing={2} container direction="column">
      ...
      <GridItem item>
        <ReactTable
          ...
          columns={columns}
          data={protocolDayList}
          setData={setProtocolDayList}
          gridInfo={gridInfo}
