...
function SrcIpGroupUpdateStatusModal({ open, close, modalParams, getStatusList }) {
  // api 호출 함수, openModal 함수
  const [apiCall, openModal] = useApi();
  // 선택한 정책 목록 상태값
  const [selectedSrcIpIdList, setSelectedSrcIpIdList] = useState([]);
  // 현재페이지 상태값
  const size = 10;
  ...
  const [srcIpList, setSrcIpList] = useState([]);
  const [isLoading, setIsLoading] = useState(true);
  const [isSaving, setIsSaving] = useState(false);
  const [isSearching, setIsSearching] = useState(false);

  const updateSrcIp = useCallback(async () => {
    if (isLoading) {
      return;
    }
    ...
    const params = {
      id: id,
      srcIpIdList: selectedSrcIpIdList,
    };
    setIsSaving(true);
    const result = await apiCall(srcIpGroupStatusApi.updateMappingListWithSrcIpGroupId, params);
    setIsSaving(false);
    if (result) {
      openModal({
  ...
  // 정책 삭제 버튼 클릭 이벤트
  const handleDelete = (srcIp) => {
    setSelectedSrcIpIdList(
      selectedSrcIpIdList.includes(srcIp.id)
        ? selectedSrcIpIdList.filter((data) => data !== srcIp.id)
        : [...selectedSrcIpIdList, srcIp.id],
    );
  };

  const getSrcIpList = useCallback(
    async ({ silent = true } = {}) => {
      if (!id) return;
      const params = {
        id: id,
      ...
      try {
        ...
        const result = await apiCall(srcIpGroupStatusApi.getMappingListAllWithSrcIpGroupId, params);
        ...
        if (result) {
          ...
          unstable_batchedUpdates(() => {
            ...
            if (selectedSrcIpIdList.length <= 0) {
              const initiallySelectedIds = data
                .filter((srcIp) => srcIp.inUsed === 1)
                .map((srcIp) => srcIp.id);
              setSelectedSrcIpIdList(initiallySelectedIds);
            }
          });
        }
      } finally {
                    ...
                    options={[
                      {
                        ...
                        color: 'secondary',
                        variant: 'outlined',
                        callBack: () => setSelectedSrcIpIdList([]),
                      },
                      {
                        label: '전체 선택',
                        color: 'secondary',
                        variant: 'contained',
                        callBack: () => {
                          setSelectedSrcIpIdList(srcIpList.map((srcIp) => srcIp.id));
                        },
                      },
                    ]}
                  />
                ...
                <List dense>
                  {srcIpList.length !== 0 ? (
                    srcIpList
                      ...
                      .map((srcIp) => (
                        <Fragment key={srcIp.id}>
                          <ListItem
                            secondaryAction={
                              <IconButton
                                sx={{ '& svg': { width: '1.3rem' } }}
                                edge="end"
                                onClick={() => handleDelete(srcIp)}
                              >
                                {selectedSrcIpIdList.includes(srcIp.id) ? (
                          ...
                          >
                            <ListItemAvatar sx={{ minWidth: '40px' }}>
                              <Avatar
                                sx={{
                                  ...
                                  '& .anticon-srcIp > svg ': { width: '0.8em !important' },
                                }}
                              >
                                <UserOutlined />
                              </Avatar>
                            ...
                            <ListItemText
                              ...
                              primary={`${srcIp.name} / ${srcIp.value} `}
                            />
                          </ListItem>
                          <Divider />
                        </Fragment>
              ...
              <GridItem item colSpan={2} sx={{ height: '430px', overflow: 'auto' }}>
                <GridItem direction="row" alignItems="center" sx={{ ml: 1 }}>
                  <GridItem direction="row" sx={{ mb: 1 }}>
                    ...
                    <Typography color="primary">{selectedSrcIpIdList.length}</Typography>
                  </GridItem>
                  <GridItem direction="row">
                    ...
                    >
                      {selectedSrcIpIdList.map((selectedSrcIp) => {
                        const srcIp = srcIpList.find((srcIp) => srcIp.id === selectedSrcIp);
                        if (srcIp)
                          return (
                            <Chip
                              ...
                              variant="contained"
                              color="secondary"
                              onDelete={() => handleDelete(srcIp)}
                              label={`${srcIp.name} / ${srcIp.value}`}
                            />
                          );
                      })}
                    </Stack>
...
SrcIpGroupUpdateStatusModal.getLayout = function getLayout(page) {
  return <Layout>{page}</Layout>;
};

export default SrcIpGroupUpdateStatusModal;
