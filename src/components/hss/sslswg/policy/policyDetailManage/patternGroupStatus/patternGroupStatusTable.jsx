...
import { setDeleteList } from '@modules/redux/reducers/hss/sslswg/pattern/patternGroupStatus';
...
function PatternGroupStatusTable(props) {
  const { getPatternGroupStatusList } = props;

  const { columns, deleteList } = useSelector((state) => state.patternGroupStatus);
  ...
  const handleUpdateColumnClick = (id) => {
    router.push({
      pathname: `/hss/sslswgManage/policyDetailManage/patternManage/update`,
      query: {
        id,
      },
    });
    ...
    <GridItem item>
      ...
      {modalOpen && (
        <PatternGroupStatusModal
          getPatternGroupStatusList={getPatternGroupStatusList}
          alertOpen={modalOpen}
...
export default PatternGroupStatusTable;
