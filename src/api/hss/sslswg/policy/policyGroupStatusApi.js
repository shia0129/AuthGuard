...
async function getMappingListAllWithPolicyGroupIdInternal(parameters, retryCount = 3) {
  const controller = new AbortController();
  const signal = controller.signal;
  try {
    const result = await policyGroupStatusApi.axios.post(
...
async function updateMappingListWithPolicyGroupIdInternal(parameters, retryCount = 3) {
  const controller = new AbortController();
  const signal = controller.signal;
  try {
    const result = await policyGroupStatusApi.axios.post(
...
const policyGroupStatusApi = {
  ...
  updateMappingListWithPolicyGroupId: updateMappingListWithPolicyGroupIdInternal,
  getMappingListAllWithPolicyGroupId: getMappingListAllWithPolicyGroupIdInternal,
};
