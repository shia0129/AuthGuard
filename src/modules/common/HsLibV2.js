...
const HsLib = {
  ...
  getGridInfo: async (listCode, api, openModal, callbackList = []) => {
    try {
      const gridInfo = await api.getGridInfo(listCode, api);
  ...
  getBeforeDate: (param, eday = HsLib.getTodayDate()) => {
    ...
    switch (gubun) {
      case 'D':
        newDate = new Date(edate.setDate(edate.getDate() - countDay)).toLocaleDateString();
        break;
      case 'M':
        newDate = new Date(edate.setMonth(edate.getMonth() - countDay)).toLocaleDateString();
        break;
      case 'Y':
        newDate = new Date(edate.setFullYear(edate.getFullYear() - countDay)).toLocaleDateString();
        break;
    }
    ...
    return `${arrNewDate[0]}-${arrNewDate[1]}-${arrNewDate[2]}`;
  },
