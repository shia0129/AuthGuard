...
function DestinationMinute() {
  ...
  const [destinationMin, setDestinationMin] = useState([
    {
      systemSeq: '시스템',
      destinationIp: '목적지ip',
  ...
  const init = async () => {
    const gridInfo = await HsLib.getGridInfo('DestinationMin', statisticsApi);
  ...
  const getDestinationMin = (parameters) => {
    // 분별 목록 조회 api 추가.
  };
  ...
  return (
    <GridItem spacing={2} container direction="column">
      <SearchInput>
        ...
        >
          ...
          <LabelInput
            ...
            //   list={systemSeq}
          />
          <LabelInput label="목적지 IP" name="endIp" />
        </GridItem>
        ...
        <Transitions type="collapse" in={searchOpenFlag}>
          ...
          >
            <LabelInput label="목적지 Port" name="endPort" />
            <LabelInput label="출발지 IP" name="startIp" />
      ...
      <GridItem item>
        <ReactTable
          listFuncName="getDestinationMin"
          columns={columns}
          data={destinationMin}
          setData={setDestinationMin}
          gridInfo={gridInfo}
...
export default DestinationMinute;
