...
function LineRealRangeChart({
  ...
  useLayoutEffect(() => {
    ...
    const loadChart = async () => {
      const { root, chart, xAxis, yAxis, legend } = await AmChart5Lib.createRealRangeXYChart({
        id,
        theme,
        legendData,
      });
      ...
      seriesData.map((item, index) => {
        const series = AmChart5Lib.createRealTimeLineSeries({
          root,
          chart,
          data,
          name: item.label,
  ...
  return <div id={id} style={{ width: '100%', height: '160px', fontSize: '10px', ...style }} />;
}

export default LineRealRangeChart;
