...
function DepartDay() {
  ...
  // 일별 목록
  const [departDayList, setDepartDayList] = useState([
    {
      systemSeq: '시스템',
      departIp: '출발지ip',
      destinationIp: '목적지ip',
  ...
  const init = async () => {
    const gridInfo = await HsLib.getGridInfo('DepartDayList', statisticsApi);
  ...
  const getDepartDayList = (parameters) => {
    // 일별 목록 조회 api 추가.
  };
  ...
  return (
    <GridItem spacing={2} container direction="column">
      ...
      <GridItem item>
        <ReactTable
          listFuncName="getDepartDayList"
          columns={columns}
          data={departDayList}
          setData={setDepartDayList}
          gridInfo={gridInfo}
