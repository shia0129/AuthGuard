...
import defaultFactoryApi from '@api/hss/common/systemManage/defaultFactoryApi';
import { AuthInstance } from '@modules/axios';
import Loader from '@components/mantis/Loader';

function DefaultFactoryManage() {
  const { instance, source } = AuthInstance();
  defaultFactoryApi.axios = instance;
  ...
  const handleSubmitDefaultFactoryConfig = async () => {
    setIsLoading(true);

    const result = await apiCall(defaultFactoryApi.defaultFactory);
  ...
  const openActionConfirmModal = () => {
    openConfirmModal({
      ...
      methods,
      onConfirm: () => handleSubmitDefaultFactoryConfig(),
    });
  };
...
DefaultFactoryManage.getLayout = function getLayout(page) {
  return <Layout>{page}</Layout>;
};

export default DefaultFactoryManage;
