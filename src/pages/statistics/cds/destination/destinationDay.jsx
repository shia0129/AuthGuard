...
function DestinationDay() {
  ...
  const init = async () => {
    const gridInfo = await HsLib.getGridInfo('DestinationDay', statisticsApi);
  ...
  const getDestinationDay = (parameters) => {
    // 일별 목록 조회 api 추가.
  };
  ...
  return (
    <GridItem spacing={2} container direction="column">
      ...
      <GridItem item>
        <ReactTable
          listFuncName="getDestinationDay"
          columns={columns}
          data={destinationDay}
          setData={setDestinationDay}
          gridInfo={gridInfo}
