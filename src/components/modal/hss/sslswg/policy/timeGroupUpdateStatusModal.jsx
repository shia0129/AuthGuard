...
function TimeGroupUpdateStatusModal({ open, close, modalParams, getStatusList }) {
  // api 호출 함수, openModal 함수
  const [apiCall, openModal] = useApi();
  // 선택한 정책 목록 상태값
  const [selectedTimeIdList, setSelectedTimeIdList] = useState([]);
  // 현재페이지 상태값
  const size = 10;
  ...
  const updateTime = useCallback(async () => {
    ...
    const params = {
      id: id,
      timeIdList: selectedTimeIdList,
    };
    setIsSaving(true);
    const result = await apiCall(timeGroupStatusApi.updateMappingListWithTimeGroupId, params);
    setIsSaving(false);
    if (result) {
      openModal({
  ...
  const handleDelete = (time) => {
    setSelectedTimeIdList(
      selectedTimeIdList.includes(time.id)
        ? selectedTimeIdList.filter((data) => data !== time.id)
        : [...selectedTimeIdList, time.id],
    );
  };
  ...
  const getTimeList = useCallback(
    async ({ silent = true } = {}) => {
      ...
      try {
        ...
        const result = await apiCall(timeGroupStatusApi.getMappingListAllWithTimeGroupId, params);
        ...
        if (result) {
          ...
          unstable_batchedUpdates(() => {
            ...
            if (selectedTimeIdList.length <= 0) {
              const initiallySelectedIds = data
                .filter((time) => time.inUsed === 1)
                .map((time) => time.id);
              setSelectedTimeIdList(initiallySelectedIds);
            }
          });
        }
      } finally {
                    ...
                    options={[
                      {
                        ...
                        color: 'secondary',
                        variant: 'outlined',
                        callBack: () => setSelectedTimeIdList([]),
                      },
                      {
                        label: '전체 선택',
                        color: 'secondary',
                        variant: 'contained',
                        callBack: () => {
                          setSelectedTimeIdList(timeList.map((time) => time.id));
                        },
                      },
                    ]}
                  />
                        ...
                        <Fragment key={time.id}>
                          ...
                          >
                            <ListItemAvatar sx={{ minWidth: '40px' }}>
                              <Avatar
                                sx={{
                                  ...
                                  '& .anticon-time > svg ': { width: '0.8em !important' },
                                }}
                              >
                                <UserOutlined />
                              </Avatar>
                            ...
                            <ListItemText
                              ...
                              primary={`${time.name} / ${formatDays(time.days)} / ${
                                time.startTime
                              } / ${time.endTime} `}
                            />
                          </ListItem>
              ...
              <GridItem item colSpan={2} sx={{ height: '430px', overflow: 'auto' }}>
                <GridItem direction="row" alignItems="center" sx={{ ml: 1 }}>
                  <GridItem direction="row" sx={{ mb: 1 }}>
                    <Typography variant="subtitle1">스케줄 정책 선택&nbsp;&nbsp;&nbsp;</Typography>
                    <Typography color="primary">{selectedTimeIdList.length}</Typography>
                  </GridItem>
                  <GridItem direction="row">
                    ...
                    >
                      {selectedTimeIdList.map((selectedTime) => {
                        const time = timeList.find((time) => time.id === selectedTime);
                        if (time)
                          return (
                            <Chip
                              ...
                              label={`${time.name} / ${formatDays(time.days)} / ${time.startTime} / ${
                                time.endTime
                              } `}
                            />
                          );
...
TimeGroupUpdateStatusModal.getLayout = function getLayout(page) {
  return <Layout>{page}</Layout>;
};

export default TimeGroupUpdateStatusModal;
