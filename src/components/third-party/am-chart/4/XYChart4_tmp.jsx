...
function XYChart4_tmp({
  ...
  const { x, seriesData, data, legendData, tooltipText } = chartData;

  useLayoutEffect(() => {
    const loadChart = async () => {
      const chart = await AmChart4Lib.hss_createXYChart({ id, data, theme, legendData });
      seriesData.map((data, index) => {
        const { label, y, fill, stroke, fillOpacity } = data;
        //console.log("#### ", id ," ## ", index, " ## ", label);
        const series = AmChart4Lib.hss_createLineSeries({
          chart,
          name: label,
          valueX: x,
          valueY: y,
