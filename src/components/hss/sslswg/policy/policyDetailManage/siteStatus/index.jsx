...
import {
  ...
  setDeleteList,
  resetState,
} from '@modules/redux/reducers/hss/sslswg/site/siteStatus';
...
import SiteStatusSearchForm from './siteStatusSearchForm';
import SiteStatusActionButton from './siteStatusActionButton';
...
function SiteStatus() {
  ...
  const init = async () => {
    const gridInfo = await HsLib.getGridInfo('SiteList', siteStatusApi);
  ...
  const getSiteStatusList = useCallback(
    async (param = {}) => {
      if (!isMount) return;
      if (!isLoading) {
        setIsLoading(true);
      ...
      const { totalElements, content } = await apiCall(siteStatusApi.getSiteStatusList, param);
  ...
  const handleDeleteButtonClick = async () => {
    if (parameterData.deleteList.length !== 0) {
      ...
      const result = await apiCall(
        siteStatusApi.deleteSiteStatusData,
      ...
      if (result) {
        openModal({
          message: result,
          onConfirm: () => {
            getSiteStatusList({ ...parameterData.parameters.current, page: 0 });
  ...
  useEffect(() => {
    ...
    if (isMount) {
      getSiteStatusList(parameterData.parameters.current);
    }
  }, [isMount, size, page]);
  ...
  return (
    <GridItem spacing={2} container direction="column">
      ...
      <SiteStatusSearchForm />
      <SiteStatusActionButton
        onSearchButtonClick={getSiteStatusList}
        onDeleteButtonClick={handleDeleteButtonClick}
      />
      {isMount && <SiteStatusTable getSiteStatusList={getSiteStatusList} />}
    </GridItem>
  );
}
