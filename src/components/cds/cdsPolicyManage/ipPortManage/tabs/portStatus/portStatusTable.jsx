...
import { setDeleteList } from '@modules/redux/reducers/portStatus';
...
function PortStatusTable(props) {
  const { getPortStatusList } = props;

  const { columns, deleteList } = useSelector((state) => state.portStatus);
  ...
  return (
    <GridItem item>
      ...
      {modalOpen && (
        <PortStatusModal
          getPortStatusList={getPortStatusList}
          alertOpen={modalOpen}
...
export default PortStatusTable;
