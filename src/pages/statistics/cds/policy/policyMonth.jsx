...
function PolicyMonth() {
  ...
  // 월별 목록
  const [policyMonthList, setPolicyMonthList] = useState([
    {
      systemSeq: '시스템',
      destinationIp: '-',
      serviceDescription: '-',
  ...
  const init = async () => {
    const gridInfo = await HsLib.getGridInfo('PolicyMonthList', statisticsApi);
  ...
  const getPolicyMonthList = (parameters) => {
    // 월별 목록 조회 api 추가.
  };
  ...
  return (
    <GridItem spacing={2} container direction="column">
      ...
      <GridItem item>
        <ReactTable
          listFuncName="getPolicyMonthList"
          columns={columns}
          data={policyMonthList}
          setData={setPolicyMonthList}
          gridInfo={gridInfo}
