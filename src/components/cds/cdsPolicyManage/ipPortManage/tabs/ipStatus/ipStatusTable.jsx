...
import { setDeleteList } from '@modules/redux/reducers/ipStatus';
import { Link } from '@mui/material';
import HsReduxTable from '@components/modules/table/HsReduxTable';
import IpStatusModal from '@components/modal/cds/cdsPolicyManage/ipPortManage/ipStatusModal';

function IpStatusTable(props) {
  const { getIpStatusList } = props;

  const { columns, deleteList } = useSelector((state) => state.ipStatus);
  ...
  const makeColumns = () =>
    columns.map((column) => ({
      ...
      Cell: (props) => {
        ...
        return reunderCodeTypeNameCell(props.value, original.name, original.id);
      },
    }));
  ...
  const reunderCodeTypeNameCell = (value, name, id) => {
    return (
      <Link
        sx={{
          cursor: 'pointer',
  ...
  return (
    <GridItem item>
      ...
      {modalOpen && (
        <IpStatusModal
          getIpStatusList={getIpStatusList}
          alertOpen={modalOpen}
