...
function DepartHour() {
  ...
  const [departHourList, setDepartHourList] = useState([
    {
      systemSeq: '시스템',
      departIp: '출발지ip',
      destinationIp: '목적지ip',
  ...
  const init = async () => {
    const gridInfo = await HsLib.getGridInfo('DepartHourList', statisticsApi);
  ...
  const getDepartHourList = (parameters) => {
    // 시간별 목록 조회 api 추가.
  };
  ...
  return (
    <GridItem spacing={2} container direction="column">
      ...
      <GridItem item>
        <ReactTable
          listFuncName="getDepartHourList"
          columns={columns}
          data={departHourList}
          setData={setDepartHourList}
          gridInfo={gridInfo}
