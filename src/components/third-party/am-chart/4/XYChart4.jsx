...
function XYChart4({
  ...
  useLayoutEffect(() => {
    const loadChart = async () => {
      const chart = await AmChart4Lib.createXYChart({ id, data, theme, legendData });

      seriesData.map((data, index) => {
        const { label, y, fill, stroke, fillOpacity } = data;
        const series = AmChart4Lib.createLineSeries({
          chart,
          name: label,
          valueX: x,
          valueY: y,
  ...
  return <div id={id} style={{ width: '100%', height: 'inherit', fontSize: '10px', ...style }} />;
}
